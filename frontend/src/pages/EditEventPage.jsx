...
import {
  ...
  export default function EditEventPage() {
    ...
    const handleUpdate = async () => {
        try {
          const base64Image = coverImage ? await fileToBase64(coverImage) : eventData.coverImage;
          const galleryBase64 = await Promise.all(galleryFiles.map(fileToBase64));
      
          // doar dacă sunt noi fișiere atașate, le procesăm
          ...
          const combinedAttachments = [
            ...(eventData.attachments || []), // păstrează-le pe cele existente
            ...attachmentsData // adaugă cele noi
          ];
      ...
      <Container maxW="container.md" py={8}>
        <VStack spacing={8}>
          ...
          <Box w="full" p={6} rounded="lg" shadow="md">
            <VStack spacing={4}>
              <Input placeholder="Event Name" value={eventData.name} onChange={(e) => handleChange("name", e.target.value)} />
              <Textarea placeholder="Description" value={eventData.description} onChange={(e) => handleChange("description", e.target.value)} />
              <Input type="date" value={eventData.date ? eventData.date.substring(0, 10) : ""} onChange={(e) => handleChange("date", e.target.value)} />
  
              <Stack w="full">
                <Heading as="h4" size="sm">Location</Heading>
                <HStack align="start" spacing={4}>
                  <Input ref={inputRef} value={eventData.location || ""} onChange={(e) => handleChange("location", e.target.value)} placeholder="Search for a new location" flex="1" />
                  {eventData.coordinates && <Box w="50%" h="200px" ref={mapRef} borderRadius="md" />}
                </HStack>
              </Stack>
  
              <Input placeholder="Tags" value={eventData.tags?.join(", ") || ""} onChange={(e) => handleChange("tags", e.target.value.split(",").map((t) => t.trim()))} />
              <Input placeholder="Max Capacity" type="number" value={eventData.capacity || ""} onChange={(e) => handleChange("capacity", Number(e.target.value))} />
              <Select value={eventData.ticketType} onChange={(e) => handleChange("ticketType", e.target.value)}>
              ...
              <Stack w="full">
                ...
                {eventData.coverImage && !coverImage && <Image src={eventData.coverImage} alt="Current cover" boxSize="200px" objectFit="cover" />}
                ...
                <Flex wrap="wrap" gap={3}>
                  {eventData.gallery?.map((imgUrl, idx) => (
                    <Box key={idx} position="relative">
                      ...
                      <CloseButton size="sm" position="absolute" top={1} right={1} onClick={() => handleRemoveGalleryImage(idx)} />
                    </Box>
                  ))}
                </Flex>
              ...
              <Stack w="full">
                <Heading as="h4" size="sm">Attachments</Heading>
                {eventData.attachments?.map((att, idx) => (
  <Flex key={idx} align="center" justify="space-between">
    <Button variant="link" color="blue.500" onClick={() => window.open(att.fileUrl, "_blank")}>
      {att.fileName}
    </Button>
