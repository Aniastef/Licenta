import {
  ...
  const EditProductPage = () => {
	...
	if (isLoading) return <Container py={10}><Spinner /></Container>;
	if (!product) return <Container py={10}><Heading size="md">Product not found.</Heading></Container>;
...
/>
    ...
    />
			  ...
			  <FormControl>
				...
				<Select 
				  ...
				  onChange={(e) => {
					...
					setProduct({ ...product, galleries: selected === "general-placeholder" ? [] : [selected] });
				  }}
				>
			  ...
			  {/* Upload new files */}
			  <FormControl><FormLabel mt={-5} >Images</FormLabel><Input type="file" accept="image/*" multiple onChange={(e) => setImageFiles([...e.target.files])} /></FormControl>
			  <FormControl><FormLabel>Videos</FormLabel><Input type="file" accept="video/*" multiple onChange={(e) => setVideoFiles([...e.target.files])} /></FormControl>
			  <FormControl><FormLabel>Audios</FormLabel><Input type="file" accept="audio/*" multiple onChange={(e) => setAudioFiles([...e.target.files])} /></FormControl>
