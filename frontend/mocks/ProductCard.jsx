...
import 'react-medium-image-zoom/dist/styles.css' // This import might not be needed if not using react-medium-image-zoom directly
...
const otherModes = modes.filter((mode) => mode !== viewMode);
  ...
  useEffect(() => {
    ...
    // Check if writing exists and is not empty (string or array)
    const hasWritingContent = (Array.isArray(product.writing) && product.writing.length > 0 && typeof product.writing[0] === "string" && product.writing[0].trim()) ||
                              (typeof product.writing === "string" && product.writing.trim());
    ...
    try {
      const res = await fetch(`/api/products/favorites/${product._id}`, {
        ...
        headers: {
          ...
          // 'credentials': 'include' is not typically set in 'headers' but as a fetch option
        },
        credentials: "include", // Set credentials here as a fetch option
      });
...
{!product ? (
  ...
  {viewMode !== "writing" &&
    ((typeof product.writing === "string" && product.writing.trim()) ||
      (Array.isArray(product.writing) && product.writing.length > 0 && product.writing[0]?.trim())) && ( // Check for non-empty string in array
